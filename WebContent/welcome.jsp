...
<body>
	...
	%>
		<form method = "post" action = "./Folder">
			...
			<input type = "hidden" name = "folderName" value =<%out.println(fold.getFolderName());%>/>
	<%
			out.println("<br/><a href=\"./welcome.jsp?parentFolderId=" + fold.getId() + "\">" + fold.getFolderName() + "</a>");
	%>
	...
	if(request.getParameter("parentFolderId") != null){
		int idParent = Controler.getParentFolderId(Integer.parseInt(request.getParameter("parentFolderId")));
		if(idParent != 0){
			out.println("<br/><a href=\"./welcome.jsp?parentFolderId=" + idParent + "\"> Retour </a>");	
		}else{
...
<!-- 	 <form name="add-by-file" enctype="multipart/form-data" method="post" action="./Upload" target="target_upload">
	...
	<script type="text/javascript">
	 ...
	 <%
     	if(request.getParameter("parentFolderId") != null){
     %>
     	<form action="upload.jsp?parentFolderId=<%out.println(request.getParameter("parentFolderId"));%>" method="post" enctype="multipart/form-data">
     <%
     	}else{
     %>		
