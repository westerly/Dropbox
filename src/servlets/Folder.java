			...
			if(action.equals("create")){
				try {
					if(!Controler.createFolder(folderName, parent)){
						request.setAttribute("errorMessage", "A problem occured during the folder creation, please be sure a folder with the same name doesn't exist and try again.");
					}
					if(Controler.isNumeric(parent)){
						this.getServletContext().getRequestDispatcher("/welcome.jsp?parentFolderId=" + parent ).forward(request, response);
					}else{
